...
const motionTarget=new MotionTarget()
...
const messages={NotAllowedError:'摄像头权限被拒绝。请在浏览器地址栏允许摄像头访问，再重新开启。',NotFoundError:'没有找到摄像头。请连接 USB 摄像头后重试。',NotReadableError:'摄像头无法读取，可能正被其他应用占用。请关闭占用应用后重试。',OverconstrainedError:'所选摄像头不可用。请选择机器狗额头相机后重试。'}
function fail(message) { stop(); error.value=message; status.value='启动或识别失败' }
async function start() {
  stop(); const token=session; error.value=''; loading.value=true; status.value='连接额头相机'
  timings.value=[]; frameCount.value=0; fpsFrames=0; lastVideoTime=-1; lastDispatch=0; history.value=[]
  try {
    status.value='正在连接机器狗额头相机'
    const health=await fetch('/api/status',{cache:'no-store'}).then(r=>r.json())
    if(!health.ready) throw new Error(health.error||'额头相机尚未供帧')
...
async function tick(now) {
  ...
  if(document.hidden||busy||!video.value||now-lastDispatch<100)return
  busy=true;lastDispatch=now;const token=session
  try {
    const response=await fetch('/api/frame',{cache:'no-store',signal:AbortSignal.timeout(5000)})
    if(!response.ok) throw new Error(await response.text())
    const bitmap=await createImageBitmap(await response.blob())
    if(token!==session) {bitmap.close();return}
    video.value.width=bitmap.width;video.value.height=bitmap.height
    video.value.videoWidth=bitmap.width;video.value.videoHeight=bitmap.height
    video.value.getContext('2d').drawImage(bitmap,0,0)
    worker.postMessage({type:'frame',bitmap,timestamp:now},[bitmap])
    watchdog=setTimeout(()=>{if(token===session)fail('识别响应超时，请重新开始识别。')},15000)
...
function pickPrimaryHand(hands) {
  const index=motionTarget.update(hands,performance.now(),video.value.videoWidth/video.value.videoHeight)
  if(motionTarget.changed){stillRps.reset();stabilizer.reset();previous='no_gesture'}
  return index
}
...
<template>
  <div class="app-shell">
    ...
    <main>
      <div class="heading"><div><div class="eyebrow">VISION / REALTIME</div><h1>让手势，被看见<span>。</span></h1><p>开始识别，用剪刀、石头、布测试实时识别。</p></div><div class="mode-control"><span>识别范围</span><el-radio-group v-model="mode" @change="resetRecognition"><el-radio-button value="rps">剪刀石头布</el-radio-button><el-radio-button value="all">全部手势</el-radio-button></el-radio-group></div></div>
      ...
      <div class="workspace">
        <section class="camera-panel">
          ...
          <div class="camera-stage">
            <canvas ref="video" :class="{mirrored:mirror}" :style="{opacity:running?1:0}" aria-label="机器狗额头相机画面"></canvas>
            <canvas ref="canvas" :class="{mirrored:mirror}" aria-label="手部关键点叠加"></canvas>
            <div v-if="!running" class="camera-empty"><div class="viewfinder"><span>⌁</span></div><h2>{{loading?'准备识别中':'把你的手势带到镜头前'}}</h2><p>{{loading?status:'连接机器狗额头相机，在电脑浏览器识别'}}</p><el-button v-if="!loading" type="primary" size="large" @click="start"><span class="button-icon">◉</span> 开始识别</el-button><el-button v-else size="large" @click="stop">取消启动</el-button><div class="empty-hands">✌️ <span>·</span> ✊ <span>·</span> 🖐️</div></div>
