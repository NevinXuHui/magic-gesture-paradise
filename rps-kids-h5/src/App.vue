...
async function start(){
  stop();const token=session;error.value='';loading.value=true;message.value='正在连接摄像头…';lastVideoTime=-1;lastDispatch=0
  try{
    if(!isSecureContext||!navigator.mediaDevices?.getUserMedia)throw Error('请从本机 localhost 或 HTTPS 打开网页。')
...
async function capture(now){
  ...
  if(document.hidden||stream?.getVideoTracks()[0]?.muted||busy||video.value.readyState<2||video.value.currentTime===lastVideoTime||now-lastDispatch<66)return
  busy=true;lastDispatch=now;lastVideoTime=video.value.currentTime;const token=session
  try{
...
function clockTick(){
  ...
  if(performance.now()-lastResult>1400){resetInteraction();message.value='等待摄像头恢复画面';return}
