...
构建后的版本：运行 `bash start-local.sh`，在 Chrome 打开 http://127.0.0.1:5174/ 。首次由管理员允许摄像头，之后孩子无需触摸屏幕。不要直接双击 index.html；摄像头需要本机 HTTP 或 HTTPS 环境。
...
- http://127.0.0.1:5174/?debug=1 显示摄像头及关键点；正式地址默认隐藏。
